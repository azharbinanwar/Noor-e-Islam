...
fun ContentPanel(config: StudioConfig, onChange: (StudioConfig) -> Unit) {
    ...
    Column(Modifier.padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            Text("Surah name (top)", color = colors.onSurface, fontSize = 12.sp, modifier = Modifier.weight(1f))
            AppSwitch(config.showSurahTop, { onChange(config.copy(showSurahTop = it)) })
        }
        Row(verticalAlignment = Alignment.CenterVertically) {
            Text("Surah name (bottom)", color = colors.onSurface, fontSize = 12.sp, modifier = Modifier.weight(1f))
            AppSwitch(config.showSurahBottom, { onChange(config.copy(showSurahBottom = it)) })
        }

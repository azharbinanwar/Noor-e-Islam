...
fun CardPanel(config: StudioConfig, swatches: List<Color>, onChange: (StudioConfig) -> Unit) {
    Column {
        ArtSlider("Opacity", config.cardColor.alpha * 100f, { onChange(config.copy(cardColor = config.cardColor.copy(alpha = it / 100f))) }, StudioDefaults.PERCENT_RANGE, "${(config.cardColor.alpha * 100).toInt()}%")
        ArtSlider("Radius", config.cardCornerRadius, { onChange(config.copy(cardCornerRadius = it)) }, StudioDefaults.CARD_RADIUS_RANGE, "${config.cardCornerRadius.toInt()}px")
